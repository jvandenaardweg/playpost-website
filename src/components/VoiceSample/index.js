...
class VoiceSample extends React.Component {
  ...
  render() {
    const { location, label, style, language, accent } = this.props
    ...
    return (
      <button
        type="button"
        className={
          isPlaying ? 'btn btn-white mb-1 mr-1' : 'btn btn-white mb-1 mr-1'
        }
        onClick={this.handlePlay}
        style={{ ...style, boxShadow: '0 2px 10px 0 rgba(0,0,0,0.17)' }}
      >
        {!isPlaying && (
          <svg
